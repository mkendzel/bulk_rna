# ---- CL vs Control: Lollipop + IFN Volcanos (LSS7T8 Liver) ----
library(tidyverse)
library(fgsea)
library(ggrepel)
library(patchwork)
...
# -- Lollipop: significant hallmark pathways
...
# Pull significant pathways from any contrast at padj < 0.05
sig_pathways <- unique(c(
  rownames(gsva_results_liver$cl_vs_control[gsva_results_liver$cl_vs_control$adj.P.Val < 0.05, ]),
  rownames(gsva_results_liver$dopc_vs_control[gsva_results_liver$dopc_vs_control$adj.P.Val < 0.05, ]),
  rownames(gsva_results_liver$dopc_vs_cl[gsva_results_liver$dopc_vs_cl$adj.P.Val < 0.05, ])
))

# Subset scores to significant pathways
scores_sig <- gsva_scores_liver[sig_pathways, ]

# Clean pathway names for display
rownames(scores_sig) <- gsub("^HALLMARK_", "", rownames(scores_sig))
rownames(scores_sig) <- gsub("_", " ", rownames(scores_sig))

# Build labels from metadata tied to each column
sample_treatment <- as.character(colData(dds_liver)$treatment[match(colnames(scores_sig), colnames(gsva_scores_liver))])
group_counts <- ave(seq_along(sample_treatment), sample_treatment, FUN = seq_along)
custom_labels <- paste0(toupper(sample_treatment), " ", group_counts)

# Scale based on actual data range
score_min <- min(scores_sig)
score_max <- max(scores_sig)

col_fun <- colorRamp2(
  c(score_min, 0, score_max),
...
Heatmap(
  scores_sig,
  col                         = col_fun,
  column_labels               = gt_render(paste0("**", custom_labels, "**")),
  column_names_rot            = 0,
  ...
  name                        = "GSVA score",
  column_title                = "Liver - GSVA Pathway Scores"
)
...
# Generate plots
p_d30  <- plot_gsea(fgsea_cl_vs_dopc,  "GSEA - Up & Down Pathways")
p_d50  <- plot_gsea(fgsea_cl_vs_mock,  "GSEA - Up & Down Pathways (d50)")
