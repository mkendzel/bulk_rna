# Redundancy QC for the Hallmark pathway calls in 6_GSEA_figures.R.
# Measures pathway-vs-pathway leading-edge overlap within each contrast and
# classifies pathway names by keyword. Flags only; nothing is dropped.
# Run from the repo root, after 4_GSEA.R.
...
# Significance cut is P_CUT from 0_config.R, shared with the figures.
...
QC_TBL_DIR <- ensure_dir(dir_res, "pathway_qc")
QC_FIG_DIR <- file.path("gsea", "pathway_qc")
...
# ---- Assessed pathway set ----
# Significant pathways for one contrast, in NES order
qc_significant <- function(exp_name, contrast_name) {
  gsea_hallmark |>
    dplyr::filter(experiment == exp_name, contrast == contrast_name,
...
# ---- Per-contrast QC ----
qc_contrast <- function(exp_name, contrast_name) {

  k  <- qc_significant(exp_name, contrast_name)
  nm <- k$pathway
  n  <- length(nm)

  if (n == 0) {
    return(list(experiment = exp_name, contrast = contrast_name, n = 0L,
                table = tibble::tibble(), pairs = tibble::tibble(),
                jaccard_le = NULL, hc = NULL, clusters = integer(0)))
  }

  # The checkpoint stores leadingEdge as one ";"-collapsed string per row.
  le <- setNames(lapply(k$leadingEdge, split_leading_edge), nm)

  missing_sets <- setdiff(nm, names(hallmark_sets))
  if (length(missing_sets) > 0) {
    message(contrast_name, ": ", length(missing_sets), " pathway(s) absent from ",
            QC_GMT_PATH, ": ", paste(missing_sets, collapse = ", "))
  }
  ...
  # Significant pathways containing each leading edge gene
  ...
  qc_tbl <- k |>
    dplyr::transmute(
      experiment, contrast, pathway, size, NES, pval, padj,
      n_leading_edge = lengths(le[pathway]),
    ...
    dplyr::select(
      experiment, contrast, pathway, size, NES, pval, padj, n_leading_edge,
  ...
  list(experiment = exp_name, contrast = contrast_name, n = n,
       table = qc_tbl, pairs = pairs, jaccard_le = ov_le$jaccard,
       hc = hc, clusters = cl)
}

# ---- Run ----
units <- purrr::map_dfr(EXPERIMENTS, function(e) {
  tibble::tibble(experiment = e, contrast = contrasts_for(e))
})

qc_all <- purrr::pmap(list(units$experiment, units$contrast), qc_contrast)
names(qc_all) <- units$contrast

for (res in qc_all) {

  if (res$n == 0) {
    message(res$contrast, ": no significant pathway - nothing to assess")
    next
  }

  utils::write.csv(res$table,
                   file.path(QC_TBL_DIR, sprintf("pathway_qc_table_%s.csv", res$contrast)),
                   row.names = FALSE)

  if (nrow(res$pairs) > 0) {
    utils::write.csv(res$pairs,
                     file.path(QC_TBL_DIR, sprintf("pathway_pair_overlap_%s.csv", res$contrast)),
                     row.names = FALSE)
  }
  ...
  if (res$n >= 2) {
    ...
    ph <- pheatmap::pheatmap(
      ...
      number_format   = "%.2f",
      fontsize_number = 6,
      main = sprintf("Leading edge Jaccard - %s (%d sets, %d clusters, cut h = %s)",
                     reg_of(res$contrast)$label, res$n, n_cl, QC_CUT_H)
    )
    save_fig(ph, paste0("le_jaccard_", res$contrast),
             width = max(7, 2 + res$n * 0.34), height = max(6, 1.5 + res$n * 0.32),
             subdir = file.path(QC_FIG_DIR, res$experiment))
  }
}

# ---- Summary ----
# One row per contrast: set, cluster and flag counts
summary_tbl <- purrr::map_dfr(qc_all, function(res) {

  reg <- reg_of(res$contrast)

  if (res$n == 0) {
    return(tibble::tibble(
      experiment = res$experiment, contrast = res$contrast, label = reg$label,
      type = as.character(reg$type), min_n = reg$min_n,
      n_sig = 0L, n_clusters = 0L, redundancy_candidates = 0L,
      tissue_mismatch = 0L, max_jaccard_le = NA_real_, largest_cluster = 0L))
  }
  ...
  tibble::tibble(
    ...
    label                 = reg$label,
    type                  = as.character(reg$type),
    min_n                 = reg$min_n,
    n_sig                 = res$n,
    n_clusters            = length(unique(res$clusters)),
    redundancy_candidates = sum(res$table$redundancy_flag == "redundancy candidate"),
    tissue_mismatch       = sum(res$table$relevance_flag == "review: tissue mismatch"),
    max_jaccard_le        = if (res$n >= 2) round(max(off, na.rm = TRUE), 3) else NA_real_,
    largest_cluster       = max(res$table$cluster_size)
  )
})

utils::write.table(summary_tbl, file.path(QC_TBL_DIR, "pathway_qc_summary.tsv"),
                   sep = "\t", quote = FALSE, row.names = FALSE)

for (i in seq_len(nrow(summary_tbl))) {
  s <- summary_tbl[i, ]
  message(sprintf(
    "%-28s %2d sets -> %2d clusters | %2d redundant | %2d tissue mismatch | max LE Jaccard %s",
    s$contrast, s$n_sig, s$n_clusters, s$redundancy_candidates, s$tissue_mismatch,
    if (is.na(s$max_jaccard_le)) "NA" else sprintf("%.2f", s$max_jaccard_le)))
}

message("Wrote QC tables to ", QC_TBL_DIR)
